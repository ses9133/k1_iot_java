...
public class J_Practice {
    public static void main(String[] args) {
        ...
        // 3) 학생들을 점수 순으로 정렬
        // : 스트림 API 중간 연산의 sorted() 와 Comparator 클래스의 .comparing 타입 정적 메서드를 활용해야함, (sorted 는
        // 클래스 형식 간의 비교가 안되기때문에)
        List<StudentClass> sortedStudents = studentClasses.stream()
                // sorted(): 클래스 객체 형식은 비교할 수없기 때문에 .ClassCastException 예외 발생 String, Integer 등은 기본정렬이 정의되어 있어 사용이 가능
        ...
        List<StudentClass> studentWithMin = studentClasses.stream()
                .filter(studentClass -> studentClass.getName().contains("민"))
                                                    // 메서드가 2개 있기때문에 메서드 참조 형식으로 바꿀 수없음.
                .collect(Collectors.toList());

        System.out.println("이름에 '민' 자가 포함된 학생");

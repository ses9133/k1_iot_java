...
/*
    === ENUM(열거형) 자료형 ===
    : 서로 관련된 '상수'들의 집합을 표현하는데 사용
    - 서로 유기적인 관계성을 가짐, 자바의 클래스로 취급
    - 의미있는 값들과 로직을 함께 담을 수 있는 구조로 활용 (가독성, 유지보수성 향상)
...
enum CoffeeSize {
    SMALL(100), MEDIUM(200), LARGE(300); // 각 필드는 CoffeeSize(enum 필드)과 같음
    // >> 생성자 호출과 비슷한 의미
...
public class B_Enum {
    public static void main(String[] args) {
        ...
        // : Enum명.데이터;
        // >> 각 상수는 정의된 Enum 타입으로 간주됨
        Fruits apple = Fruits.APPLE;
        System.out.println(apple);

        FruitsClass fruit = new FruitsClass("이승아");
        System.out.println(fruit); // FruitsClass(name=이승아) 과일이름에 사람이름이 올 수 없도록 만드는게 ENUM

        // == ENUM 의 주요 메서드 === //
        // 1) values() : 열거형의 모든 값을 배열로 반환
        System.out.println(Fruits.values()); // 배열의 주소값

        for(Fruits fruitVal : Fruits.values()) {
            System.out.print(fruitVal + " ");
        }
        System.out.println();

        // 2) name(): ENUM 의 상수를 문자열로 반환
        String appleName = Fruits.APPLE.name();
        System.out.println(appleName); // APPLE

        // 3) valueOf(String name) : 문자열을 Enum 상수로 반환
        // : 잘못된 문자열 전달시 IllegalArgumentException 발생
        Fruits orange1 = Fruits.valueOf("ORANGE");
      // Fruits orange2 = Fruits.valueOf("Orange"); -> 대소문자까지 확인함
        ...
        CoffeeSize another = CoffeeSize.LARGE;
        System.out.println(another + " 's ml: " + another.getMl() + " ml");

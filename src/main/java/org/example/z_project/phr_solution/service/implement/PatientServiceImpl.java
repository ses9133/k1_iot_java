...
public class PatientServiceImpl implements PatientService {
    ...
    private static long currentId = 1; // 환자 고유 번호
    ...
    public void registerPatient(PatientCreateRequestDto dto) {
        // 환자 등록
        // 1) 요청, 응답
        //      - 요청값 필요. 응답은 필요에 따라 결정
        // 2) 요청값이 있을 경우, 데이터 검증
        //      - ex) 해당 환자 정보가 이미 저장되어있는지 또는 잘못된 이름, 나이, 성별인지 확인
    ...
    public List<PatientListResponseDto> listAllPatients() {
        ...
        List<PatientListResponseDto> responseDtos = patients.stream()
                .map(p -> new PatientListResponseDto(

...
// === 클래스 == //
// : 파일 (코드 저장소)
// : 한개의 .java 파일에는 하나의 클래스가 포함(파일명과 동일)
// : 모든 코드가 클래스 안에서 동작됨
public class A_Array { //  클래스명 == 파일명  (파일명은 대문자로 시작!! + UpperCamelCase 사용)
    ...
    public static void main(String[] args) {
        ...
        // - 데이터를 "연속적으로" 저장
        // - 순서 있음(index - 0부터 시작하여 1씩 증가되는 값)
        // - 중복 가능(동일 값을 여러 요소에 저장 가능) cf) 요소: 배열에서 각각의 데이터를 의미
        ...
        // cf) 참조 자료형 - 주소값이 저장
        // : 데이터 배열이 저장된 첫 번째 메모리 주소가 저장 (인덱스번호 0 번의 주소)
        ...
        // cf) 배열 생성시 모든 요소는 해당 데이터 타입의 기본값으로 초기화
        // >> 숫자(0), 문자(빈문자), 논리형(false), String(null)

        // 배열의 요소에 접근하여 데이터 변경(삽입)
        scores1[0] = 80;
        scores1[1] = 90;
        scores1[2] = 85;    // 전체 요소의 개수 - 1 : 마지막 index 번호!!
        System.out.println(scores1[0]);
        System.out.println(scores1[1]);
        System.out.println(scores1[2]);
      //  System.out.println(scores1[3]); ArrayIndexOutOfBoundsException 에러
        ...
        // >> 배열의 선언과 동시에 초기화
        // : new 연산자 사용 + 직접적으로 요소를 대입하여 배열 생성시 길이 지정 X
        // >> 요소의 개수를 파악하여 길이가 자동 지정됨
        ...
        // cf) 참조 자료형의 출력
        // : 자바에서 객체를 sout 으로 출력하면 내부적으로 객체.toString() 기능을 호출
        // > 모든 참조 자료형의 집합체
        String a = "안녕";
        System.out.println(a);  // .toString() 이 생략되어있음.
        System.out.println(a.toString());
        // >> String 은 내부적으로 toString() 호출시 실제 데이터값 자체를 반환하도록 설정되어있음.(char 마찬가지)
        // >> 배열, 객체는 toString() 호출시 주소값을 문자열로 출력하도록 되어있음.

        int[] arr1 = {1, 2, 3};
        char[] arr2 = { 'a', 'b', 'c' };
        System.out.println(arr1);   // [I@28a418fc
        System.out.println(arr2);   // abc - char 는 문자열처럼 자동처리됨.

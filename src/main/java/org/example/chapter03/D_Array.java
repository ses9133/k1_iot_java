...
// Array: 같은 데이터타입의 값들이 순차적으로 저장되는 자료 구조
// cf) Arrays 클래스
...
public class D_Array {
    public static void main(String[] args) {
        ...
        // 1. 배열 정렬 (오름 차순): Arrays.sort(정렬할 배열);
        Arrays.sort(numbers);
        System.out.println(numbers[0]);

        // cf) . (마침표) 연산자: 해당 클래스 내부의 기능을 사용

        // 2. 배열 변환(배열 내용을 문자열로 출력): Arrays.toString(변한할 배열)
        System.out.println(numbers);
        System.out.println(Arrays.toString(numbers));

        // 3. 배열검색: Arrays.binarySearch (배열, 검색 내용);
        // - 배열내에 존재 하면: 존재하는 요소의 인덱스 번호를 반환
        // - 배열내에 없으면 : 해당 검색값의 음수를 반환(음수 반환)

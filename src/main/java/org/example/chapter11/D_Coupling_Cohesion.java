...
/*
    ...
    >> 모듈의 독립성을 결합도(coupling) 과 응집도(cohesion)의 기준단계로 측정

    1. 결합도(낮은 결합도)
        : 두 개의 클래스나 모듈이 얼마나 강하게 연결되어있는지 나타냄
        >> 모듈과 모듈 간의 의존도
        ...
        >> 낮은 결합도
            : 프로그램 유지보수에 용이
            - 새로운 기능을 추가하거나 기존 코드 수정시 오류 가능성을 낮춤

    2. 응집도(높은 응집도) (=클래스의 자아)
...
class ClassRoom {
    private Teacher teacher;    // 의존 역전 원칙

    // 의존성 주입(DI)
    // : 클래스가 직접 객체를 생성 X, 외부에서 객체를 주입받도록 설계
...
// >> IoC 컨테이너가 해당 역할을 담당
// >> ClassRoom 이 직접 객체 생성 X, 외부에서 주입 방식(IoC 예시)
...
public class D_Coupling_Cohesion {
    public static void main(String[] args) {
        ...
        // ClassRoom 이 교사 객체를 직접 생성하지 않고, 외부에서 주입
        // >> ClassRoom 이 어떤 선생님이 오든 신경 쓸 필요가 없음
        ClassRoom koreanClass = new ClassRoom(koreanTeacher);
        ClassRoom mathClass = new ClassRoom(mathTeacher);

        StudentCohesion student = new StudentCohesion("정은혜");

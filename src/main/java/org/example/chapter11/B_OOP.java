...
// 1) 잘못된 설계 예시 - 필요하지 않은 행위를 강제로 구현하게 시킴
interface Worker {
    void work();
    void eat();
}
...
// > 인터페이스가 SRP(단일 책임 원칙)도 위반하고 ISP 도 위반한 형태임
...
class Employee implements Workable, Eatable {
    @Override
    public void eat() {
        System.out.println("13시부터 14시까지 점심시간 입니다.");
    }
...
// 1) 잘못된 설계 예시 - MathBook 의 구체적인 클래스에 의존함.
//  >> 다른 책으로 변경하려면 코드 자체를 수정해야함.
class MathBook {
    void read(){
